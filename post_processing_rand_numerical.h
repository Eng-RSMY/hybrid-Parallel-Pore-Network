...
Teuchos::Array<global_ordinal_type>vol_ind(counter);
Teuchos::Array<scalar_type>vol_val(counter);
size_t num;
    ...
    for(int j=0;j<counter;j++)
      {
        ...
        values_current[countN]=abs(x_1d(myGlobalElements[i])-x_1d(mat_ind[j])+surf)*mat_val[j];
...
//Teuchos::Array<scalar_type> mat_val(counter);
K->getGlobalRowCopy(myGlobalElements[i],mat_ind,mat_val,num);
Vol_frac->getGlobalRowCopy(myGlobalElements[i],vol_ind,vol_val,num);
...
global_ordinal_type countN=0;
    ...
    for(int j=0;j<counter;j++)
      {
  //         std::cout<<"vol"<<vol_val[j]<<std::endl; 
        values_current[countN]=abs(x_1d(myGlobalElements[i])-x_1d(mat_ind[j])+surf)*mat_val[j];
        values_X[countN]=(1e-8)+sqrt(vol_val[j]*vol_val[j]+2*vol_val[j]*values_current[countN]*dt);
        indices_current[countN]=mat_ind[j];
        countN=countN+1;
...
Vol_frac->resumeFill();
Vol_frac->replaceGlobalValues(myGlobalElements[i], indices_current, values_X);
